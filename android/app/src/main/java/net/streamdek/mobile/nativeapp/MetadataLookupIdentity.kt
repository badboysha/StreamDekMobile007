...
internal object MetadataLookupIdentity {
  ...
  fun imdbId(value: String): String? =
    Regex("^(?:imdb:)?(tt[0-9]{6,12})(?::[0-9]+(?::[0-9]+)?)?$", RegexOption.IGNORE_CASE)
      .matchEntire(value.trim())?.groupValues?.get(1)?.lowercase()

  fun tmdbId(value: String): String? =
    Regex("^(?:tmdb:(?:(?:movie|tv):)?)?([0-9]{1,12})$", RegexOption.IGNORE_CASE)
      .matchEntire(value.trim())?.groupValues?.get(1)
}
